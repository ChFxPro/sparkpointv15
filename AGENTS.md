...
- Rural Health Convening landing page is available at `/rural-health-convening`, with a priority registration list for the limited 200-seat event, a live seats-left counter at the top of the registration panel (gated by `SHOW_RURAL_HEALTH_SEATS_TICKER` in `src/data/opsStatus.ts`, currently `true`; it renders only once `rh-ticket-stock` returns a count and stays hidden while loading or on error, and the one count covers both the $65 and $40 `IMPACT-REG` rates because they share one Squarespace inventory), partner/sponsor attribution, and a public introduction to the Rural Health Field Simulator. Sponsor logos link to each org's website; sponsor billing follows five active tiers — Summit ($3,500: UNC Health Pardee, Transylvania Regional Hospital, equal size/prominence), Ridgeline ($2,500: Pisgah Health Foundation, visibly smaller/secondary), Highlands ($1,500: Transylvania County Tourism Development Authority, tertiary), Foothills (Dogwood Health Trust, Vaya Health, First Citizens Bank, Hendersonville Pediatrics), and Friends (AdventHealth, UnitedHealthcare, Comporium, lowest billing) — reflected in the hero credit line and partnership-section logo hierarchy. Each tier is one array in `RuralHealthConveningPage.tsx` (`summitSponsors` … `friendsSponsors`) rendered in both places; per-logo height overrides in `ruralHealthConvening.css` (keyed on `rh-logo-stage-*`) exist because stacked/multi-line lockups need more box height than single-line wordmarks to read at the same optical weight, so tier order alone does not determine rendered size.
- Thrive @ Five: Common Ground Release Party is available at `/events/thrive-at-five`, with the July 31 event details, August coffee fundraiser, August 1 purchase CTA, performance video, and Resilience Hub story. The August profit-share has ended; the page remains as a record, and it no longer holds a homepage promo slot.
- The homepage promo row (`src/components/HomePromoShowcase.tsx`) pairs the active story collection card with a date-gated event card. As of 2026-09-09 that event card is `RuralHealthPromo.tsx` (links to `/rural-health-convening`, styled with that page's paper/forest/brass system, self-retires after 2026-10-01; its "Registration open" kicker carries a clay `N left` seats badge driven by the same `useRemainingSeats` hook and `SHOW_RURAL_HEALTH_SEATS_TICKER` flag as the convening page, hidden until a live count arrives, switching to `Registration full` / `Waitlist` at 0). Both cards share a band structure (kicker, headline, rule, lede, three-cell facts row, full-width button, footnote) so they balance; the story card's facts row and footnote come from optional `promoFacts`/`promoFootnote` fields on `StoryCollection` in `src/data/storyCollections.ts`, so give any new active collection its own values rather than reusing another's.
